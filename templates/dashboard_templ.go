...
func DashboardPage(taskLists []tasks.TaskList, activeListID string, activeTasks []tasks.Task, hideCompleted bool) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 11, "</aside></div><script>\n\t\t\tfunction openSidebar() {\n\t\t\t\tdocument.getElementById('sidebar').classList.remove('-translate-x-full');\n\t\t\t\tdocument.getElementById('sidebar-backdrop').classList.remove('hidden');\n\t\t\t}\n\t\t\tfunction closeSidebar() {\n\t\t\t\tdocument.getElementById('sidebar').classList.add('-translate-x-full');\n\t\t\t\tdocument.getElementById('sidebar-backdrop').classList.add('hidden');\n\t\t\t}\n\t\t\tfunction closeDetail() {\n\t\t\t\tvar p = document.getElementById('detail-panel');\n\t\t\t\tp.classList.add('hidden');\n\t\t\t\tp.classList.remove('fixed','inset-0','z-50','w-full');\n\t\t\t}\n\t\t\t// On mobile, show detail-panel as full-screen overlay when content loads\n\t\t\tdocument.body.addEventListener('htmx:afterSwap', function(evt) {\n\t\t\t\tif (window.innerWidth >= 768) return;\n\t\t\t\tvar t = evt.detail.target;\n\t\t\t\tif (t.id === 'detail-panel') {\n\t\t\t\t\t// Check if it's real content (not the empty placeholder)\n\t\t\t\t\tif (t.querySelector('form')) {\n\t\t\t\t\t\tt.classList.remove('hidden','w-96','border-l');\n\t\t\t\t\t\tt.classList.add('fixed','inset-0','z-50','w-full');\n\t\t\t\t\t} else {\n\t\t\t\t\t\tcloseDetail();\n\t\t\t\t\t}\n\t\t\t\t}\n\t\t\t});\n\t\t</script>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
